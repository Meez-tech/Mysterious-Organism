...
let pAequorFactory = (specimenNum, dna) => {
  return {
    ...
    dna: dna,
    mutate() {
      // method that changes 1 random base
      console.log(
      ...
      while (random === dna[index]) {
        // loop until new random base is diff from the old base
    ...
    compareDNA(pAequorObj) {
      // method to compare current organism with the previous one
      let counter = 0;
      for (let i = 0; i < this.dna.length; i++) {
        if (i == 0) {
          // show the bases of the two organisms just one time each
          console.log(
        ...
        if (this.dna[i] === pAequorObj.dna[i]) {
          // if the two bases are equal
          console.log(
            this.dna[i] + " and  " + pAequorObj.dna[i] + " are equal"
          ); // print message
    ...
    willLikelySurvive() {
      // returns true if at least 60% of its bases are C or G
      let counter = 0; // counter value
      for (let i = 0; i < this.dna.length; i++) {
        // looping over bases of caller object
        if (this.dna[i] == "C" || this.dna[i] == "G") {
          // if the base if C or G...
          counter += 1; // ...increase counter
        }
      }
...
let pool = () => {
  ...
  while (sampleArray.length < 30) {
    // loop until adaptingSpecimen array contains 30 elements
...
console.log(
  `--Invoking the pool function (creating a pool of 30 surviving samples)--`
);
console.log(pool());
